...
:precondition (and (posn ?x ?p) (available ?x) (posn ?y ?p) (on ?y ?z) (clear ?y))
:effect (and (not (posn ?y ?p)) (lifting ?x ?y) (not (clear ?y)) (not (available ?x)) 
             (clear ?z) (not (on ?y ?z)) (increase (fuel-cost) 1)))

(:action Drop 
:parameters (?x - hoist ?y - crate ?z - surface ?p - place)
:precondition (and (posn ?x ?p) (posn ?z ?p) (clear ?z) (lifting ?x ?y))
:effect (and (available ?x) (not (lifting ?x ?y)) (posn ?y ?p) (not (clear ?z)) (clear ?y)
		(on ?y ?z)))

(define (problem GROUNDED-PATHWAYS-01)
 (:domain GROUNDED-PATHWAYS-PROPOSITIONAL)
 (:init
  (not-chosen_e2f13)
  (not-chosen_p130-e2f5p1-dp12)
  (num-subs_l0)
  (not-chosen_p16)
  (not-chosen_prbp2)
  (not-chosen_chk1)
  (not-chosen_raf1)
  (not-chosen_cdc25c)
  (not-chosen_cdk46p3-cycdp1)
  (not-chosen_ap2)
  (not-chosen_dmp1)
  (not-chosen_p300)
  (not-chosen_cdk7)
  (not-chosen_cdk46p3-cycd)
  (not-chosen_pcaf)
  (not-chosen_prb-e2f4p1-dp12)
  (not-chosen_sp1)
  (valid)
  (cpt (unlistedeffect_ASSOCIATE_PRBP1P2_AP2_PRBP1P2-AP2_available_prbp1p2-ap2) 0.5)
  (cpt (possclob_ASSOCIATE_PRBP1P2_AP2_PRBP1P2-AP2_available_prbp1p2) 0.5)
  (cpt (possclob_ASSOCIATE_PRBP1P2_AP2_PRBP1P2-AP2_available_ap2) 0.5)
  (cpt (openprec_ASSOCIATE-WITH-CATALYZE_CDC25C_CHK1_CDC25CP2_available_chk1) 0.5)
  (cpt (possclob_ASSOCIATE-WITH-CATALYZE_CDC25C_CHK1_CDC25CP2_available_cdc25c) 0.5)
  (cpt (openprec_ASSOCIATE-WITH-CATALYZE_DMP1_CDK46P3-CYCD_DMP1P1_available_cdk46p3-cycd) 0.5)
  (cpt (possclob_ASSOCIATE-WITH-CATALYZE_DMP1_CDK46P3-CYCD_DMP1P1_available_dmp1) 0.5)
  (cpt (openprec_ASSOCIATE-WITH-CATALYZE_DMP1_CDK46P3-CYCDP1_DMP1P1_available_cdk46p3-cycdp1) 0.5)
  (cpt (possclob_ASSOCIATE-WITH-CATALYZE_DMP1_CDK46P3-CYCDP1_DMP1P1_available_dmp1) 0.5)
  (cpt (openprec_ASSOCIATE-WITH-CATALYZE_PRBP2_CDK46P3-CYCDP1_PRBP1P2_available_cdk46p3-cycdp1) 0.5)
  (cpt (possclob_ASSOCIATE-WITH-CATALYZE_PRBP2_CDK46P3-CYCDP1_PRBP1P2_available_prbp2) 0.5)
  (cpt (openprec_ASSOCIATE-WITH-CATALYZE_PRBP2_CDK46P3-CYCD_PRBP1P2_available_cdk46p3-cycd) 0.5)
  (cpt (possclob_ASSOCIATE-WITH-CATALYZE_PRBP2_CDK46P3-CYCD_PRBP1P2_available_prbp2) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_P16_CDK7_P16-CDK7_available_p16-cdk7) 0.5)
  (cpt (possclob_ASSOCIATE_P16_CDK7_P16-CDK7_available_cdk7) 0.5)
  (cpt (possclob_ASSOCIATE_P16_CDK7_P16-CDK7_available_p16) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_PCAF_P300_PCAF-P300_available_pcaf-p300) 0.5)
  (cpt (possclob_ASSOCIATE_PCAF_P300_PCAF-P300_available_pcaf) 0.5)
  (cpt (possclob_ASSOCIATE_PCAF_P300_PCAF-P300_available_p300) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_PRBP2_AP2_PRBP2-AP2_available_prbp2-ap2) 0.5)
  (cpt (possclob_ASSOCIATE_PRBP2_AP2_PRBP2-AP2_available_prbp2) 0.5)
  (cpt (possclob_ASSOCIATE_PRBP2_AP2_PRBP2-AP2_available_ap2) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_RAF1_P130-E2F5P1-DP12_RAF1-P130-E2F5P1-DP12_available_raf1-p130-e2f5p1-dp12) 0.5)
  (cpt (possclob_ASSOCIATE_RAF1_P130-E2F5P1-DP12_RAF1-P130-E2F5P1-DP12_available_raf1) 0.5)
  (cpt (possclob_ASSOCIATE_RAF1_P130-E2F5P1-DP12_RAF1-P130-E2F5P1-DP12_available_p130-e2f5p1-dp12) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_RAF1_PRB-E2F4P1-DP12_RAF1-PRB-E2F4P1-DP12_available_raf1-prb-e2f4p1-dp12) 0.5)
  (cpt (possclob_ASSOCIATE_RAF1_PRB-E2F4P1-DP12_RAF1-PRB-E2F4P1-DP12_available_prb-e2f4p1-dp12) 0.5)
  (cpt (possclob_ASSOCIATE_RAF1_PRB-E2F4P1-DP12_RAF1-PRB-E2F4P1-DP12_available_raf1) 0.5)
  (cpt (unlistedeffect_ASSOCIATE_SP1_E2F13_SP1-E2F13_available_sp1-e2f13) 0.5)
  (cpt (possclob_ASSOCIATE_SP1_E2F13_SP1-E2F13_available_sp1) 0.5)
  (cpt (possclob_ASSOCIATE_SP1_E2F13_SP1-E2F13_available_e2f13) 0.5)
 )
 (:goal 0.000001
 (and (done)))
)

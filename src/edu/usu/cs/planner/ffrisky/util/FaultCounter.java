	...
	public static void initialize(Domain domain, Problem problem) 
	{
		...
		try
		{
			...
			for (Fault risk : allRisks) 
			{
				...
				{
					System.out.println("In FaultCounter.initialize(2 params). trying again");
					e.printStackTrace();
...
//		return FaultCounter.bdd.getSetCount(bdd);
//	}
...
//		
//		
////		int notdd = FaultCounter.bdd.not(faultSet);
////		FaultCounter.bdd.ref(notdd);	
//		BigInteger unsolvableDomains = getBigUnSolvableDomainCount(brs.getFaults());		
////		FaultCounter.bdd.deref(notdd);
//		//System.out.print("]");
...
//	}
	...
	private static void usage() {
		logger.debug("usage: FaultCounter [domain file] [problem file] [ffrisky output file] [stats output file] [instance] [probability]");
	}
}

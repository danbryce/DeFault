	...
	protected void setPropositionFaults(FaultStateNode faultStateNode, IncompleteActionInstance action) {
		...
		for(Proposition p : effects){
			...
			if(action.getAddEffects().contains(p)){
				...
				//possible add
				Fault addf = Fault.getRiskFromIndex(Fault.POSSADD, action.getName(), p.getName());
			...
			if(action.getDeleteEffects().contains(p)){
				//known del
					...
					action.getPossibleAddsDeletes().contains(p)){
				//possible del
				Fault delf = Fault.getRiskFromIndex(Fault.POSSDEL, action.getName(), p.getName());
			...
			FaultSet persistence = faultStateNode.state.get(p);
			if(persistence == null){

...
public class BatchTester_Test_PlannerRawPerformance 
{
		...
		for(int gridSize = 2; gridSize <= 2; gridSize*=2) //2-32
		{
			...
			{	
				for(int version = 3; version <= 3; version++)//1-3
				{
					...
					{
						for(int simSeed = 38; simSeed < 1000 && Test_PlannerRawPerformance.numSuccesses < 1; simSeed++)
						{	
							...
							try{ Test_PlannerRawPerformance.main(args); }

...
public class BatchTester_Simulation_PassiveLearningAgent 
{
	...
	{				
		...
		System.out.println("E: planner changed bdd, index error occurred.\n");
		
		//Just mess around with the for loop values...
		for(int gridSize = 4; gridSize <= 4; gridSize*=2) //2-32
		{
			for(double bridgeDensity = 0.5; bridgeDensity <= .5; bridgeDensity += 0.25) //0.0 - 1.0
			{	
				for(int version = 1; version <= 1; version++)//1-3
				{
					for(int numFile = 1; numFile <= 1; numFile++) //numFiles 1-10
					{
						for(int simSeed = 3; simSeed < 4 && Simulation_PassiveLearningAgent.numSuccesses < 10; simSeed++)
						{	

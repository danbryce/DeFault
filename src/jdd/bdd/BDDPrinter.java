...
public class BDDPrinter {
	...
	public static String printSetToString(int bdd, int max, NodeTable nt, NodeName nn)  {
		StringBuilder sb = new StringBuilder();
		
		if( bdd < 2) {
			if(nn != null)  sb.append( (bdd == 0) ? nn.zero() : nn.one() );
			else sb.append( (bdd == 0) ? "FALSE\n" : "TRUE\n");
		} else {
			...
			sb = printSetToString_rec(bdd, 0, sb);
			//sb.append("\n");
			helpGC();
		}
		return sb.toString();
	}
	private static StringBuilder printSetToString_rec(int bdd, int level, StringBuilder sb) {

		if(level == set_chars_len) {
			if(nn == null) {
				for(int i = 0; i < set_chars_len; i++)
					sb.append(set_chars[i]);
			} else {
				for(int i = 0; i < set_chars_len; i++)
					if(set_chars[i] == '1')
						sb.append(" " + nn.variable(i));
			}
		...
		if(var > level || bdd == 1 ) {
			set_chars[level] = '-';
			sb = printSetToString_rec(bdd, level+1, sb);
			return sb;
		}
		...
		if(low != 0) {
			set_chars[level] = '0';
			sb = printSetToString_rec(low, level+1, sb);
		}

		if(high != 0) {
			set_chars[level] = '1';
			sb =printSetToString_rec(high, level+1, sb);
		}
		return sb;
	}

	
	public static String toString(int bdd, int max, NodeTable nt, NodeName nn)  {
		StringBuilder sb = new StringBuilder();
		
		if( bdd < 2) {
			if(nn != null)  JDDConsole.out.println( (bdd == 0) ? nn.zero() : nn.one() );
			else sb.append((bdd == 0) ? "FALSE" : "TRUE");//JDDConsole.out.println( (bdd == 0) ? "FALSE" : "TRUE");
		} else {
			...
			sb.append(toString_rec(bdd, 0));
			sb.append("\n");//JDDConsole.out.println();
			helpGC();
		}
		return sb.toString();
	}
	private static String toString_rec(int bdd, int level) {
		StringBuilder sb = new StringBuilder();
		if(level == set_chars_len) {
			if(nn == null) {
				for(int i = 0; i < set_chars_len; i++)
					sb.append(set_chars[i]);//JDDConsole.out.print(set_chars[i]);
			} else {
				for(int i = 0; i < set_chars_len; i++)
					if(set_chars[i] == '1')
						sb.append(" " + nn.variable(i));//JDDConsole.out.print(" " + nn.variable(i));
			}
		...
		if(var > level || bdd == 1 ) {
			set_chars[level] = '-';
			sb.append(toString_rec(bdd, level+1));
			return sb.toString();
		}
		...
		if(low != 0) {
			set_chars[level] = '0';
			sb.append(toString_rec(low, level+1));
		}

		if(high != 0) {
			set_chars[level] = '1';
			sb.append(toString_rec(high, level+1));
		}
		return sb.toString();
	}
}

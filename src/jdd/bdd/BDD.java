...
public class BDD extends NodeTable {
	...
	protected BigIntegerCache big_sat_cache;
	...
	public BDD(int nodesize, int cache_size) {
		...
		big_sat_cache = new BigIntegerCache("BIGSAT", cache_size / Configuration.bddSatcountDiv);
	...
	protected void post_removal_callbak() {
		...
		big_sat_cache.invalidate_cache(); // NO NEED TO GROW THIS ONE ?
	 ...
	 * @see #and(int, int)
	 */
	public int and(List<Integer> args) {
		if (args.size() == 2) {
			return and(args.get(0), args.get(1));
		}
		
		int last = args.remove(args.size() - 1);
		return and(and(args), last);
	}
	 ...
	 * @see #or(int, int)
	 */
	public int or(List<Integer> args) {
		if (args.size() == 2) {
			return or(args.get(0), args.get(1));
		}
		
		int last = args.remove(args.size() - 1);
		return or(or(args), last);
	}
	 ...
	 * terminal nodes!
	 */
	public BigInteger bigSatCount(int bdd) {

		// if the number of variables has changed since the last time, the cache is invalid!
		if(last_sat_vars != -1 && last_sat_vars != num_vars) big_sat_cache.invalidate_cache();
		last_sat_vars = num_vars;

		BigInteger v1 = BigInteger.valueOf(2).pow(getVar(bdd));
		return  v1.multiply(bigSatCount_rec(bdd));
	}

	protected BigInteger bigSatCount_rec(int bdd) {
		if(bdd < 2) return BigInteger.valueOf(bdd);

		if(big_sat_cache.lookup(bdd)) return big_sat_cache.answer;
		int hash = big_sat_cache.hash_value;

		int low = getLow(bdd);
		int high = getHigh(bdd);

		BigInteger ret = 
			bigSatCount_rec(low).multiply(
					BigInteger.valueOf(2).pow(getVar(low)  - getVar(bdd)  -1)).add(
							bigSatCount_rec(high).multiply(
									BigInteger.valueOf(2).pow(getVar(high) - getVar(bdd)  -1)));

		big_sat_cache.insert(hash, bdd, ret);
		return ret;
	}
	...
	public long getMemoryUsage() {
		...
		if(big_sat_cache != null) ret += big_sat_cache.getMemoryUsage();
	...
	public String printSetToString(int bdd) {	return BDDPrinter.printSetToString(bdd, num_vars, this, null);	}
	public String toString(int bdd) {	return BDDPrinter.toString(bdd, num_vars, this, null);	}
	...
	public BigInteger getBigSetCount(int bdd) {
		return BDDCounter.getBigSetCount(bdd, num_vars, this);
	}

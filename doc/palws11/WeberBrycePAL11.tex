...
\title{Goal and Failure Directed  Learning about Incomplete Actions}
...
incompleteness, learn by asking questions of a domain expert, or learn through
trial and error.  In deciding what to learn, the agent must consider what
...
how to select the fewest questions.  Agents that learn by trial and error can
reach scenarios where it is impossible to learn about actions that impact goal
...
state, and to know why the action failed (i.e., that the possible preconditions
are actual preconditions), it would need to apply the action again in $n$ different states where only one
of each of the preconditions is not satisfied.


Our approach to formulating questions relies on both goal and failure directed
reasoning.  Failure directed reasoning focuses on diagnosing which incomplete
features of actions led to an unsatisfied precondition of a previously executed
action. Goal directed reasoning identifies which incomplete features may cause
future failure while executing a plan.  Some of the issues that arise when
combining goal and failure directed reasoning for identifying questions are:
\begin{packed_itemize}
\item Whether asking questions is at all necessary, or if learning by acting is
enough for agents to achieve their goals?
\item Will    
\end{packed_itemize}


Plan failures occur because action preconditions are not
satisfied
...
% \begin{algorithm}
 ...
 \While{}{
  ...
  $\phi \leftarrow$ GoalDirectedKA$(d(\pi))$; \qquad\CommentSty{//Optional}\\ 
  }
\While{$\pi \not= ()$ and $G\not\subseteq s$}{
...
\Repeat{$\pi \not= \pi'$}{
	 $\pi' \leftarrow {\tt plan}(s, G, {A}, \phi)$\;
	 \If{$\pi'==\pi$}{
	 $\phi \leftarrow$ FailureDirectedKA$(\phi, d(\pi))$; \CommentSty{//Optional}\\
	 }
	 }
	  $\phi \leftarrow \exists_{fail}  \phi$\;
}
}
\caption{Agent$(s, G, {A})$}\label{alg:replan}
\end{algorithm}
...
\begin{algorithm}[t]
\SetLine
\KwIn{agent knowledge $\phi$,\\ \qquad\quad plan failure explanation $d(\pi)$}

$Q_{\phi} \leftarrow \{{\sf f} | \delta \models \phi \wedge d(\pi)
, \delta \models {\sf f} \;\text{or}\; \delta \models \neg{\sf f}\}$\;
\While{$\phi?fail$ and $|PI(\phi)|>1$}{ %more than one cube
${\sf f} \leftarrow \underset{{\sf f}  \in Q_{\phi}}{\operatorname{argmax}}
\sum\limits_{\substack{\delta : \delta \models \phi,\\ \delta\models {\sf
f}}}
\frac{1}{|\{{\sf f}| \delta \models {\sf f}\}|^2}$\; 
$\phi \leftarrow {\tt filter}(\phi, {\tt Ask}({\sf f}))$\;
$Q_{\phi} \leftarrow Q_{\phi} \backslash {\sf f}$
}
\Return \phi\;
\caption{FailureDirectedKA$(\phi, d(\pi))$}\label{alg:failuredirected}
\end{algorithm}


\begin{algorithm}[t]
\SetLine
\KwIn{plan failure explanation $d(\pi)$}

$Q_{d(\pi)} \leftarrow \{{\sf f} | \delta \models d(\pi)
, \delta \models {\sf f} \;\text{or}\; \delta \models \neg{\sf f}\}$\;
\While{$|PI(d(\pi))| > 1$}{ %more than one cube
${\sf f} \leftarrow \underset{{\sf f}  \in Q_{d(\pi)}}{\operatorname{argmax}}
\sum\limits_{\substack{\delta : \delta \models \phi,\\ \delta\models {\sf
f}}}
\frac{1}{|\{{\sf f}| \delta \models {\sf f}\}|^2}$\; 
$\phi \leftarrow {\tt filter}(\phi, {\tt Ask}({\sf f}))$\;
$Q_{d(\pi)} \leftarrow Q_{d(\pi)} \backslash {\sf f}$
}

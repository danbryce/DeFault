...
% \begin{algorithm}
 $\phi \leftarrow \top$;  $\pi \leftarrow null $\;
 $\pi \leftarrow {\tt getPlanWithKA}(s, G, {A}, \phi, \pi, isOneQ)$\;
\While{$\pi \not= ()$ and $G\not\subseteq s$}{
...
\caption{For PALWS11. Goalie-KA$(s, G, {A})$}\label{alg:replan}
\end{algorithm}
...
\SetLine
\KwIn{state $s$, goal $G$, actions ${A}$,\\ 
\hspace{9.5 mm} agent knowledge $\phi$, plan $\pi$ , bool isOneQ}

 $\pi' \leftarrow {\tt getPlan}(s, G, {A}, \phi)$\;
\While{conditions}
{
	$\psi \leftarrow {\tt mergeSources}(\phi, \pi')$\;
	${\sf f} \leftarrow {\tt getBestQFromPICubes}(\psi)$\;
	
	\If{${\sf f} \neq null$}
	{
		 $\phi \leftarrow {\tt filter}(\phi, {\tt Ask}({\sf f}))$\;
	}
	\If{${\sf f} = null $ or isOneQ$ $}
	{
		 $\pi' \leftarrow {\tt getPlan}(s, G, {A}, \phi)$\;
	}
}
\Return $\pi'$\;
\caption{getPlanWithKA $(s, G, {A}, \phi, \pi,isOneQ)$} 
\label{alg:getPlanWithKA}
\end{algorithm}

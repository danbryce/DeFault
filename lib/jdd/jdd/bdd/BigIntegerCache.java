...
public final class BigIntegerCache extends CacheBase {
	private int []in;
	 ...
	 */
	public BigIntegerCache(String name, int size) {
		super(name);
		...
		out = Allocator.allocateBigIntegerArray(cache_size);
		Array.set(in, -1);
	...
	private void grow_and_invalidate_cache() {
		...
		out = null;	out = Allocator.allocateBigIntegerArray(cache_size);
		Array.set(in, -1);
		possible_bins_count = 0;
		num_clears++;
	}
	...
	public void insert(int hash, int key1, BigInteger value) {
		possible_bins_count++;
		in[hash] = key1;
		out[hash] = value;
	}
